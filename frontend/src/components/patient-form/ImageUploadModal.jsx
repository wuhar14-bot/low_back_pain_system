    ...
    try {
      ...
      for (let i = 0; i < uploadedImages.length; i++) {
        ...
        try {
          ...
          // Parse the extracted medical data
          let medicalData;
          if (result.response && typeof result.response === 'string') {
            try {
              medicalData = JSON.parse(result.response);
            } catch (e) {
              medicalData = {};
              console.warn('Failed to parse OCR medical data, using raw text');
            }
          } else {
            medicalData = {};
          }
          ...
          // Add raw OCR text to remarks if exists
          if (result.raw_text) {
            const remarkText = `图片${i + 1}(${image.name}): ${result.raw_text.slice(0, 200)}${result.raw_text.length > 200 ? '...' : ''}`;
