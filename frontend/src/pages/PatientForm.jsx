...
export default function PatientForm() {
    ...
    SECTIONS.forEach(section => {
      ...
      if (formKeys.some(key => {
        ...
        return true;
         ...
         if(section.id === 'basic' && patientData.study_id) {
           completed.add(section.id);
         } else if(section.id !== 'basic') {
           completed.add(section.id);
         }
      }
    });
  ...
  const handleDataExtracted = (extractedData) => {
    try {
      ...
      const completed = new Set();
      SECTIONS.forEach(section => {
        if (section.id === 'basic' && (sanitizedData.name || sanitizedData.age || sanitizedData.gender || sanitizedData.phone || sanitizedData.study_id)) {
          completed.add(section.id);
